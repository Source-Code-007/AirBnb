...
const Homepage = () => {
    const [activeCategory, setActiveCategory] = useState('countryside')
                ...
                <TabList className="flex gap-2 justify-center items-center flex-wrap mb-8">
                    {
                        categories?.map((category, ind) => {
                            return <Tab key={ind} className={`px-4 py-2 bg-green-500 font-bold text-slate-50 text-xl cursor-pointer rounded`} onClick={() => setActiveCategory(category)}>{category}</Tab>
                        })
                    }
                </TabList>
